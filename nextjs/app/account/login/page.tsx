  ...
  return (
    <div className="flex flex-col bg-white rounded-md overflow-hidden shadow-xl w-[350px]">
      <div className="flex w-full bg-gradient-to-t from-sky-700 via-sky-800 to-sky-700">
          <p className="text-white ml-2 text-sm">Login</p>
      </div>
      <Form action="/user/login" formMethod="POST" formEncType="multipart/form-data">
          <div className="flex flex-col items-center bg-white rounded-md p-4">
              ...
              <button id="loginbutton" 
                className="flex p-1 pl-2 pr-2 text-center text-white rounded-md shadow-sm bg-sky-800 active:bg-sky-600 hover:bg-sky-700 transition-all"
              >
                Login
              </button>
          </div>
          <a href="/restore" target="_self">Forgot password</a>
          <a href="/register" target="_self">Register account</a>
      </Form>
    </div>
  );
}