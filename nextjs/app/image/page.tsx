  ...
  return (
    <div className="flex flex-col bg-white rounded-md overflow-hidden shadow-xl w-[350px]">
        <div className="flex w-full bg-gradient-to-t from-sky-700 via-sky-800 to-sky-700">
            <p className="text-white ml-2 text-sm">Image</p>
        </div>
        <form action="/image/api" method="POST" encType="multipart/form-data">
            <div className="bg-white rounded-md p-4">
                {/* <input type="hidden" name="id" value={ id }/> */}
                <input type="file" name="image" accept="image/jpeg" className="rounded-md mb-2 border-2 border-solid border-gray-400" required/>
                <div className="flex flex-row justify-between items-start">
                    <button id="uploadbox" className="flex p-1 pl-2 pr-2 text-center text-white rounded-md shadow-sm bg-sky-800 active:bg-sky-600 hover:bg-sky-700 transition-all">Upload image</button>
        ...
        <p className="p-1">5MB max, jpeg only</p>
    </div>
  );
}
